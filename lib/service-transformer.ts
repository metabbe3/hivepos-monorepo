  ...
  for (const svc of services) {
    ...
    if (!groupMap.has(key)) {
      groupMap.set(key, {
        ...
        // User-assigned group wins (so "Kelola Grup" + the Group dropdown drive the tabs);
        // fall back to the name-derived category when no group is assigned.
        category: svc.group?.name || getServiceCategory(baseName),
        variants: [],

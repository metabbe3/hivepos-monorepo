...
export function getDateRangePreset(key: DateRangePreset): { from: string; to: string } {
  ...
  // Local calendar YYYY-MM-DD — NOT toISOString(), which converts to UTC and shifts WIB
  // (UTC+7) dates back a day (e.g. lastMonth resolving to May 31 instead of Jun 1).
  const fmt = (d: Date) =>

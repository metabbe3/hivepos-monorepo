...
function KanbanOrderCard({
  ...
  return (
    <Card
      className="cursor-pointer border border-border/50 bg-card shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-[shadow,transform]"
      tabIndex={0}
      role="button"
          ...
          <div className="flex items-center gap-1">
            {order.customerPhone && (
              <Button
                variant="ghost"
                size="icon"
                className="h-9 w-9 text-green-600 hover:text-green-700 hover:bg-green-50 dark:hover:bg-green-950/30"
                onClick={(e) => {
                  e.stopPropagation();
    ...
    <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory pb-4 sm:grid sm:grid-cols-2 xl:grid xl:grid-cols-4 sm:overflow-visible sm:pb-0">
      {COLUMNS.map((col) => {
        ...
        return (
          ...
          >
            {/* Column header */}
            <div className={`flex items-center gap-2 px-3 py-2.5 rounded-xl ${col.headerBg}`}>

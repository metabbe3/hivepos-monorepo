...
async function checkAndMaybeNuke() {
  try {
    ...
    localStorage.setItem(STORAGE_KEY, serverNonce);
    // Cache-bust: append the new nonce as a query param so the browser fetches fresh HTML
    // (bypassing the disk cache). Works for BOTH web + PWA users — not just SW-managed caches.
    const u = new URL(window.location.href);
    u.searchParams.set("_bust", serverNonce);

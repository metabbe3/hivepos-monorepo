...
// Null-safe: hivepos-api customers omit customerStatus — default instead of crashing.
export function statusGradient(status: CustomerStatus | undefined): string {
  return (status && CUSTOMER_COLORS[status]?.gradient) ?? "from-border to-border";
}

export function statusAvatarBg(status: CustomerStatus | undefined): string {
  return (status && CUSTOMER_COLORS[status]?.avatarBg) ?? "bg-muted";
}

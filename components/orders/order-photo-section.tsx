...
import { apiFetch, apiRaw, ApiClientError } from "@/modules/shared";
...
export function OrderPhotoSection({ orderId }: Props) {
  ...
  async function upload(kind: PhotoKind, file: File) {
    ...
    try {
      // apiRaw — keeps the FormData body intact (apiFetch would JSON-stringify it).
      const form = new FormData();
      form.append("kind", kind);
      form.append("file", file);
      const res = await apiRaw(`/api/orders/${orderId}/photos`, {
        method: "POST",
        body: form,
      });

...
import { apiFetch, apiRaw } from "@/modules/shared";
...
export function MonthlyPnlReport() {
  ...
  async function handleExport() {
    setExporting(true);
    try {
      const res = await apiRaw(`/api/reports/monthly-pnl/export?month=${month}&year=${year}`);
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {/* PEMASUKAN */}
          <div>
 <h4 className="font-semibold text-sm mb-2">Pemasukan</h4>
          ...
          {/* PENGELUARAN */}
          <div>
 <h4 className="font-semibold text-sm mb-2">Pengeluaran</h4>
            <div className="space-y-1">
 ...
 <th className="text-left py-2 px-2 text-xs text-muted-foreground font-semibold w-20">Hari</th>
 <th className="text-center py-2 px-1 text-xs text-muted-foreground font-semibold w-8">Tgl</th>
 <th className="text-left py-2 px-2 text-xs text-muted-foreground font-semibold">Pelanggan</th>
 <th className="text-right py-2 px-2 text-xs text-muted-foreground font-semibold w-16">KG</th>
 <th className="text-left py-2 px-2 text-xs text-muted-foreground font-semibold">Satuan</th>
 <th className="text-right py-2 px-2 text-xs text-muted-foreground font-semibold w-28">Jumlah</th>
 <th className="text-right py-2 px-2 text-xs text-muted-foreground font-semibold w-28">Total Hari</th>
 <th className="text-right py-2 px-2 text-xs text-muted-foreground font-semibold w-28">Kumulatif</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    day.orders.map((order, oi) => {
                      ...
                      return (
                        <tr
                          ...
                          `}
                        >
 <td className={`py-1.5 px-2 ${isFirst ? "text-xs font-bold text-foreground": ""}`}>
                            {isFirst ? day.dayName : ""}
                          </td>

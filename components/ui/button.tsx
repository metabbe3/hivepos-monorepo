  ...
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-[color,background-color,border-color,box-shadow,transform] outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 disabled:cursor-not-allowed aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      ...
      size: {
        ...
        touch: "h-11 gap-2 rounded-xl px-4 text-[0.95rem] font-semibold [&_svg:not([class*='size-'])]:size-4",
        icon: "size-8",
        // ponytail: 28px (size-7) — dense token/chip rows; below WCAG 2.5.5 AAA
        // 44px but the safe floor for tightly-packed inline icon buttons.
        "icon-xs":
          "size-7 rounded-[min(var(--radius-md),10px)] in-data-[slot=button-group]:rounded-lg [&_svg:not([class*='size-'])]:size-3",
        // ponytail: 32px (size-8) — below WCAG 2.5.5 AAA 44px, but the smallest
        // safe floor for dense toolbar icon buttons (edit/delete rows).
        "icon-sm":

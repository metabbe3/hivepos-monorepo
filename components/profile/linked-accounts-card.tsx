...
export function LinkedAccountsCard({
  ...
  async function handleLink() {
    setLinking(true);
    try {
      // Bearer-gated: backend sets a link cookie + returns the Google consent URL.
      const { data } = await apiFetch<{ url: string }>("/api/auth/google/link", { method: "POST" });
      if (data?.url && typeof window !== "undefined") {
        window.location.href = data.url; // → Google → callback links → /profile?linked=google
      } else {
  ...
  async function handleUnlink() {
    ...
    setUnlinking(true);
    try {
      await apiFetch("/api/auth/google/unlink", { method: "DELETE" });

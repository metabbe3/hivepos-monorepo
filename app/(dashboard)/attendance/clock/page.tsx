...
export default function AttendanceClockPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setPin(""); // clear the pad so the user sees feedback and can retry
    } finally {
      setSubmitting(false);
    }
  };
      ...
      >
        <div className="relative flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="text-sm text-background/50" suppressHydrationWarning>{dateStr}</p>
          </div>
          <p className="sa-tnum font-mono text-4xl font-bold tracking-wider tabular-nums sm:text-5xl" suppressHydrationWarning>{timeStr}</p>
        </div>
      </div>
 ...
 <p className="mb-3 text-xs font-bold text-emerald-50">

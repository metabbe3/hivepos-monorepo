...
export function CustomerPicker() {
  ...
  async function handleCreateCustomer(e?: React.FormEvent) {
    ...
    try {
      ...
      // Duplicate phone → backend 409. Locate + auto-select the existing
      // customer instead of leaving the user stuck (the cached list may not
      // contain them, so client-side search alone can't recover). Server-search
      // is uncapped, so it finds them across the tenant.
      if (
        online &&
        ...
        try {
          ...
          >(`/api/customers?search=${encodeURIComponent(custForm.phone.trim())}`);
          const norm = (p: string) => (p ?? "").replace(/\D/g, "");
          const want = norm(custForm.phone.trim());
          // ponytail: first exact normalized-phone match. Refine to
          // branch-scoped if cross-branch duplicate phones ever cause ambiguity.
          const match = (data ?? []).find((c) => want !== "" && norm(c.phone) === want);
          if (match) {
            ...
            setCustomers((prev) =>
              prev.some((c) => c.id === existing.id) ? prev : [...prev, existing],
            );
            setSelectedCustomer(existing);
            setCreating(false);
            setCustForm({ name: "", phone: "" });
            toast.success(t("newOrder.customerAlreadySelected"));
            return;
          }
        } catch {

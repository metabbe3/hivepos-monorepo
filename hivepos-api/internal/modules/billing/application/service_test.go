...
func (f *fakeRepo) GetPlanByTier(_ context.Context, _ string) (*domain.Plan, error) {
	return f.plan, nil
}
...
func TestGetStatus_NoSubscription(t *testing.T) {
	r := &fakeRepo{}
	got, err := application.NewService(r, "", "", true).GetStatus(context.Background(), "t1")
...
func TestGetStatus_Found(t *testing.T) {
	...
	got, _ := application.NewService(r, "", "", true).GetStatus(context.Background(), "t1")
...
func TestCheckout_PlanNotFound(t *testing.T) {
	r := &fakeRepo{}
	if _, err := application.NewService(r, "", "", true).Checkout(context.Background(), application.CheckoutInput{PlanID: "p1", Months: 1, BranchIDs: []string{"b1"}}, "t1"); err == nil {
...
func TestCheckout_HappyCreatesPendingPayment(t *testing.T) {
	...
	res, err := application.NewService(r, "", "", true).Checkout(context.Background(), application.CheckoutInput{PlanID: "p1", Months: 1, BranchIDs: []string{"b1"}}, "t1")
...
func TestWebhook_MissingSignature(t *testing.T) {
	// No MIDTRANS_SERVER_KEY + unsigned (AllowUnsignedWebhook=false) ⇒ default-deny.
	if err := application.NewService(&fakeRepo{}, "", "", false).HandleWebhook(context.Background(), application.WebhookInput{}); err == nil {
		t.Fatal("unsigned webhook must be rejected when verification unavailable")
	}
}

func TestWebhook_PaymentNotFound(t *testing.T) {
	r := &fakeRepo{}
	if err := application.NewService(r, "", "", true).HandleWebhook(context.Background(), application.WebhookInput{SignatureKey: "x", OrderID: "o1"}); err == nil {
...
func TestWebhook_NonSuccessIsNoop(t *testing.T) {
	...
	if err := application.NewService(r, "", "", true).HandleWebhook(context.Background(), application.WebhookInput{SignatureKey: "x", OrderID: "o1", TransactionStatus: "deny"}); err != nil {
		t.Fatal(err)
	}
	if r.settledOrder != "" {
...
func TestWebhook_SuccessActivates(t *testing.T) {
	r := &fakeRepo{payment: &domain.SaaSPayment{ID: "pay1", TenantID: "t1"}}
	if err := application.NewService(r, "", "", true).HandleWebhook(context.Background(), application.WebhookInput{SignatureKey: "x", OrderID: "o1", TransactionStatus: "settlement"}); err != nil {
		t.Fatal(err)
	}
...
func TestValidatePromo_NotFound(t *testing.T) {
	r := &fakeRepo{}
	res, _ := application.NewService(r, "", "", true).ValidatePromo(context.Background(), application.PromoValidateInput{Code: "X"})

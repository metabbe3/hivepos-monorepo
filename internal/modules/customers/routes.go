...
func (m *Module) list(w http.ResponseWriter, req *http.Request) {
	...
	if p, err := strconv.Atoi(req.URL.Query().Get("page")); err == nil { filter.Page = p }
	if l, err := strconv.Atoi(req.URL.Query().Get("limit")); err == nil { filter.Limit = l }

	list, total, err := m.svc.List(req.Context(), tenantID, filter)
	if err != nil { apphttp.Error(w, http.StatusInternalServerError, err.Error()); return }
...
func (m *Module) create(w http.ResponseWriter, req *http.Request) {
	...
	if !decodeJSON(w, req, &input) { return }
	if input.Name == "" { apphttp.ValidationError(w, "name is required"); return }

	var clientID *string
	if cid := req.Header.Get("X-Client-Id"); cid != "" { clientID = &cid }
	input.ClientID = clientID

	c, err := m.svc.Create(req.Context(), input, tenantID, branchID)
	if err != nil {
		if err.Error() == "customer with this phone already exists" {
			apphttp.Error(w, http.StatusConflict, err.Error()); return
		}
		apphttp.Error(w, http.StatusInternalServerError, err.Error()); return
	}
	apphttp.Created(w, c)
}
...
func (m *Module) getByID(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil { apphttp.NotFoundError(w, "Customer not found"); return }
	apphttp.Success(w, c)
}
...
func (m *Module) topUpDeposit(w http.ResponseWriter, req *http.Request) {
	...
	if !decodeJSON(w, req, &body) { return }
	if body.Amount <= 0 { apphttp.ValidationError(w, "amount must be positive"); return }
	if body.Type == "" { body.Type = "TOP_UP" }

	d, err := m.svc.TopUpDeposit(req.Context(), chi.URLParam(req, "id"), middleware.GetTenantID(req), body.Amount, body.Type, body.Notes)
	if err != nil { apphttp.Error(w, http.StatusInternalServerError, err.Error()); return }
	apphttp.Created(w, d)
}
...
func decodeJSON(w http.ResponseWriter, req *http.Request, dst interface{}) bool {
	if err := json.NewDecoder(req.Body).Decode(dst); err != nil {
		apphttp.ValidationError(w, "Invalid JSON body"); return false
	}
	return true
}

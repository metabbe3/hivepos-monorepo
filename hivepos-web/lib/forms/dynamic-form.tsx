...
function useAsyncOptions(field: FieldDef) {
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    // ponytail: use apiFetch (attaches Bearer + unwraps envelope). Raw fetch() here
    // hit /api/roles etc. without the token → 401 → empty select options. Fix: authed fetch.
    apiFetch<unknown>(field.optionsEndpoint)
      .then(({ data }) => {
        if (cancelled) return;
...
export function DynamicForm({
  ...
  const [values, setValues] = React.useState<Record<string, unknown>>(() => {
    ...
    // ponytail: seed `id` from recordId/initialData so id-conditional fields
    // (e.g. staff password: condition !values.id, validate !all.id) behave on EDIT.
    // Without this, edit mode looked like create → password field showed + validated.
    const editId = recordId ?? initialData?.id;

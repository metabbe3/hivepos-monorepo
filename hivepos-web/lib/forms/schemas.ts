...
export const stockItemSchema: FormSchema = {
  ...
  // ponytail: hivepos-api rejects string values for numeric fields (Go float64 decode → 400
  // "Invalid JSON body"). Coerce empty→0 and strings→numbers at submit, matching service/expense.
  transform: (values) => ({
    ...values,
    currentQuantity: values.currentQuantity === "" ? 0 : Number(values.currentQuantity),
    lowStockThreshold: values.lowStockThreshold === "" ? 0 : Number(values.lowStockThreshold),
    purchasePricePerUnit: values.purchasePricePerUnit === "" ? 0 : Number(values.purchasePricePerUnit),
  }),
};
...
export const staffSchema: FormSchema = {
  ...
  // ponytail: hivepos-api POST /api/users requires the `role` enum (OWNER|MANAGER|EMPLOYEE)
  // in addition to roleId; the form only captures roleId. Default role to EMPLOYEE on create.
  // Upgrade path: add a system-role selector that maps to the enum.
  transform: (values) => ({ ...values, role: values.role ?? "EMPLOYEE" }),
};

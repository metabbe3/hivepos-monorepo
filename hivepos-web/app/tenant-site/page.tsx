...
// Public tenant lookup via Go (endpoint pending — PORT-DEBT §2; null → notFound).
async function fetchTenant(slug: string): Promise<TenantPublicData | null> {
  try {
    const { data } = await apiFetch<TenantPublicData>(`/api/public/tenants/${slug}`);
    return data ?? null;
  } catch {
    return null;
  }
...
function extractKelurahan(address: string | null | undefined): string {
...
function pricingLabel(type: string | null | undefined): string {
...
export default async function TenantSitePage() {
  ...
  // Public services for this tenant (Go endpoint pending — PORT-DEBT §2; graceful empty).
  let services: { name: string; description?: string | null; pricingType?: string; basePrice?: number | string }[] = [];
  try {
    const { data } = await apiFetch<{
      services: { name: string; description?: string | null; pricingType?: string; price?: number | string }[];
                        ...
                        return (
                          ...
                          >
                            <span className={`text-sm ${isToday ? "text-[#0284C7]" : "text-[#334155]"}`}>
                              {h.day}
 {isToday && <span className="ml-2 text-[11px] text-[#0284C7]">Hari ini</span>}
                            </span>

...
export function PromoCodesManager() {
  ...
  const fetchCodes = useCallback(() => {
    setLoading(true);
    apiFetch<PromoCode[]>("/api/super-admin/promo-codes")
      .then((r) => {
        setCodes(r.data ?? []);
      })
      .catch((err) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editing) {
        await apiFetch(`/api/super-admin/promo-codes/${editing.id}`, {
          method: "PATCH",
          body: {
        ...
        const { data } = await apiFetch<PromoCode>("/api/super-admin/promo-codes", {
          method: "POST",
          body: {
  ...
  async function handleToggle(code: PromoCode) {
    try {
      const { data } = await apiFetch<PromoCode>(`/api/super-admin/promo-codes/${code.id}`, {
        method: "PATCH",
        body: { isActive: !code.isActive },
      });
      toast.success(`${code.code} ${data.isActive ? "diaktifkan" : "dinonaktifkan"}.`);
      fetchCodes();
    } catch (err) {
      toast.error(err instanceof ApiClientError ? err.message : "Terjadi kesalahan.");
    }

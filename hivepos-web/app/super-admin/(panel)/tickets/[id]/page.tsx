...
import { MessageSquare, Star, Building2, Clock, Tag, AlertTriangle, Loader2 } from "lucide-react";
...
export default function TicketDetailPage() {
  ...
  const reload = useCallback(() => {
    if (!id) return;
    apiFetch<Ticket>(`/api/super-admin/tickets/${id}`)
      ...
      >
        ...
        <div className="mb-6 rounded-xl bg-card p-6 ring-1 ring-foreground/10 shadow-sm">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-muted-foreground">
            ...
            {ticket.submitterPhone && <span className="text-xs">· {ticket.submitterPhone}</span>}
            <span className="flex items-center gap-1.5">
              <Clock className="h-3.5 w-3.5" />
              {formatDateTime(ticket.createdAt)}
            </span>
            {ticket.resolvedAt && <span>· Resolved {formatDateTime(ticket.resolvedAt)}</span>}
            {ticket.tenantId && (
              <span className="flex items-center gap-1.5">
                <Building2 className="h-3.5 w-3.5" />
                <Link href={`/super-admin/tenants/${ticket.tenantId}`} className="font-medium text-primary hover:underline">
          ...
          {ticket.csatRating != null && (
            <div className="mt-4 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-900/60 dark:bg-amber-950/30">
              ...
              <div className="text-sm">
                ...
                {ticket.csatComment && <p className="mt-1 italic text-muted-foreground">"{ticket.csatComment}"</p>}
              </div>
            </div>
          )}
        </div>
          ...
          <div className="lg:col-span-2">
            <DetailSection title={`Thread (${ticket.comments.length})`} icon={MessageSquare}>
                      ...
                      <li key={c.id} className="relative">
                        <span
                          className={`absolute -left-[27px] top-1 flex h-4 w-4 items-center justify-center rounded-full ring-4 ring-background ${
                            isTenant ? "bg-sky-500" : isStaff ? "bg-emerald-500" : "bg-muted-foreground"
                          }`}
                        />
                        <div
              ...
              <div className="mt-6">
                ...
                <TicketReplyBox ticketId={ticket.id} onReplied={reload} />

...
import type { Stats, HeatmapData, ComparisonMetric } from "@/components/dashboard/dashboard-types";
import type { PaymentMethod } from "@/app/generated/prisma/enums";

// hivepos-api /api/dashboard/stats may return a partial shape (some nested objects omitted).
...
function normalizeStats(d: Partial<Stats> | null | undefined): Stats {
  ...
  return {
    ...
    todayOmset: s.todayOmset ?? 0,
    omsetChange: s.omsetChange ?? null,
    previousRevenue: s.previousRevenue ?? 0,
    ...
    lowStock: s.lowStock ?? [],
    customerInsights: (s.customerInsights ?? {}) as Stats["customerInsights"],
    comparison: (s.comparison ?? {}) as Stats["comparison"],
    unpaidDelivered: s.unpaidDelivered ?? 0,
    unpaidOrders: s.unpaidOrders ?? [],
    turnaround:
      s.turnaround ?? { avgHours: null, fastestHours: null, slowestHours: null, completedCount: 0 },
    ...(s.sparkline ? { sparkline: s.sparkline } : {}),
  };
}

// hivepos-api /api/dashboard/stats returns a leaner shape than pos-saas's Stats
// (totalOrders/totalCustomers/totalRevenue/totalExpenses/ordersByStatus/paymentBreakdown).
// Map it into the full Stats so the dashboard renders real data. Fields Go doesn't provide
// (change %, top customers, recent orders, cash-flow detail) default safely.
function mapGoDashboardStats(g: Record<string, unknown>): Stats {
  const obs = (g.ordersByStatus ?? {}) as Record<string, number>;
  const pb = (g.paymentBreakdown ?? g.paymentMethodBreakdown ?? {}) as Record<string, number>;
  const revenue = Number(g.totalRevenue ?? 0);
  const expenses = Number(g.totalExpenses ?? 0);
  const orders = Number(g.totalOrders ?? 0);
  const cmp = (current: number): ComparisonMetric => ({ current, previous: 0, changePercent: null });
  return {
    todayOrders: orders,
    inProgress: Number(obs.IN_PROGRESS ?? 0),
    readyForPickup: Number(obs.READY ?? 0),
    ...
    topCustomers: [],
    serviceBreakdown: [],
    paymentMethodBreakdown: Object.entries(pb).map(([method, total]) => ({
      method: method as PaymentMethod,
      count: 0,
      total: Number(total),
    })),
    recentOrders: [],
    cashFlow: { income: revenue, expenses, net: revenue - expenses, walletDeposits: 0 },
    orderPipeline: {
      RECEIVED: Number(obs.RECEIVED ?? 0),
      IN_PROGRESS: Number(obs.IN_PROGRESS ?? 0),
      READY: Number(obs.READY ?? 0),
      DELIVERED: Number(obs.DELIVERED ?? 0),
    },
    lowStock: [],
    customerInsights: {
      total: Number(g.totalCustomers ?? 0),
      newThisWeek: 0,
      active: 0,
      atRisk: 0,
      lapsed: 0,
    ...
    comparison: {
      ...
      netCashFlow: cmp(revenue - expenses),
    },
    unpaidDelivered: 0,
    unpaidOrders: [],
    turnaround: {
      ...
      fastestHours: null,
      slowestHours: null,
      completedCount: Number(obs.DELIVERED ?? 0),
    },
  };
}

// Resolve whatever /api/dashboard/stats returns into a renderable Stats (or null → graceful state).
function toStats(data: unknown): Stats | null {

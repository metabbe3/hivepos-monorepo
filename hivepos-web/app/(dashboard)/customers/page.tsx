...
function CustomersContent() {
  ...
  function refresh() {
    apiFetch<CustomerListItem[]>(`/api/customers?${buildParams()}`)
      .then(({ data }) =>
        setCustomers(
          (data ?? []).map((c) => ({ ...c, customerStatus: c.customerStatus ?? "ACTIVE" })),
        ),
      )
      .catch(() => setCustomers([]));
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-border/60 bg-muted/30 p-3 sm:flex-row sm:items-center">
        ...
        >
          <SelectTrigger className="h-9 w-[140px]" aria-label={t("customers.filterByStatus")}>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>

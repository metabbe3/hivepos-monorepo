    ...
    (id: string) => {
      apiFetch<BranchDetail>(`/api/branches/${id}`)
        .then(async (r) => {
          setBranch({
            ...r.data,
            // hivepos-api omits counts + users; default so branch.counts.* / branch.users don't crash.
            counts: r.data.counts ?? { orders: 0, services: 0, customers: 0, users: 0 },
            users: r.data.users ?? [],
          });
          // hivepos-api branch detail omits assigned staff → load from /api/users.
          try {
            const { data: all } = await apiFetch<{ id: string; name: string; email: string; role: string; branchId?: string | null }[]>("/api/users");
            const staff = (all ?? []).filter((u) => u.branchId === id);
            setBranch((b) => (b ? { ...b, users: staff } : b));
          } catch {

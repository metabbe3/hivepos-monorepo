...
async function fetchOnce(): Promise<KanbanOrder[]> {
  ...
  inflight = apiFetch<KanbanOrder[]>("/api/dashboard/kanban")
    .then(({ data }) => {
      // hivepos-api returns status AGGREGATES ([{status,count,sum}]) here, not order rows.
      // Only accept items that look like real orders (have id); else empty so KanbanBoard
      // renders "no orders" instead of crashing/key-warning on aggregate objects.
      const orders =
        Array.isArray(data) && data.every((o) => o && typeof (o as KanbanOrder).id === "string")
          ? (data as KanbanOrder[])
          : [];
      cache = { orders, ts: Date.now() };
